...
namespace EWS\Component\DoctrineEncryptBundle\DependencyInjection\Compiler;
...
class RegisterServiceCompilerPass implements CompilerPassInterface {
    ...
    public function process(ContainerBuilder $container) {
        ...
        if ($container->hasParameter('ews_doctrine_encrypt.encryptor_service')) {
            // Load some parameters
            $secretKey = $container->getParameter('ews_doctrine_encrypt.secret_key');
            $encryptorServiceId = $container->getParameter('ews_doctrine_encrypt.encryptor_service');

            // Get the definitions
            $subscriberDefinition = $this->getDefinition($container, 'ews_doctrine_encrypt.subscriber');
