...
namespace EWS\Component\DoctrineEncryptBundle\DependencyInjection;
 ...
 */
class EWSDoctrineEncryptExtension extends Extension {
    ...
    public function load(array $configs, ContainerBuilder $container) {
        ...
        $supportedEncryptorClasses = array('aes256' => 'EWS\Component\DoctrineEncryptBundle\Encryptors\EWSEncryptor');
        ...
        $container->setParameter('ews_doctrine_encrypt.encryptor_class_name', $encryptorFullName);
        $container->setParameter('ews_doctrine_encrypt.secret_key', $config['secret_key']);

        if (!empty($config['encryptor_service'])) {
            $container->setParameter('ews_doctrine_encrypt.encryptor_service', $config['encryptor_service']);
        }
